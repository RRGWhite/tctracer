 ...
 *
 * Create ctt/jacoco_out, ctt/classes and ctt/output directories first.
 * Coverage data only needs to be generated once per project.
 *
 * The key function in this class is runOn, which takes a list of test names, project base directory,
 * class filter (directory format), class files path relative to base directory, and source files path
 * relative to base directory. The runOn* functions feed appropriate parameters to the runOn function
...
public class CTTCoverage {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<String> testNames = hitSpectra.stream()
                .map(hs -> hs.cls + "#" + hs.func.substring(0, hs.func.lastIndexOf('(')))
    ...
    public static void runOn(List<String> testNames, String baseDir, String filterStr, String relativeClassFilesPath, String relativeSourceFilesPath) throws Exception {
        for (String testName : testNames) {
            String execFile = run(baseDir, testName);
            List<String> relevantClasses = parse(baseDir, execFile, filterStr);
            String filteredClassPath = copyClassesToTempDir(baseDir, testName, relevantClasses, relativeClassFilesPath).toString();
    ...
    public static String run(String workingDirectoryPath, String testCase) throws Exception {
        ...
        if (!workingDirectory.exists()) throw new Exception("Invalid directory");

        String destinationExecFile = String.format("ctt/jacoco_out/%s.exec", testCase);
        String jacocoAgentArgLine = String.format("%s=append=false,destfile=%s", JACOCO_AGENT_PATH, destinationExecFile);

        String commandLine = String.format("%s surefire:test -Dtest=%s -DargLine=\"-javaagent:%s\"", MAVEN_PATH, testCase, jacocoAgentArgLine);
    ...
    public static List<String> parse(String workingDirectoryPath, String jacocoExecPath, String filterStr) {
    ...
    public static Path copyClassesToTempDir(String baseDir, String testName, List<String> classList, String relativeClassFilesPath) throws IOException {
    ...
    // Exports report
    public static void exportReport(String workingDirectoryPath, String jacocoExecPath, String filteredClassFilesPath, String sourceFilesPath, String testName) throws Exception {
        File workingDirectory = new File(workingDirectoryPath);
        if (!workingDirectory.exists()) throw new Exception("Invalid directory");

        Files.createDirectories(Paths.get(workingDirectoryPath, "ctt/output"));

        String commandLine = String.format("java -jar \"%s\" report \"%s\" --classfiles \"%s\" --sourcefiles \"%s\" --html \"ctt/output/%s\" --xml \"ctt/output/%s.xml\"", JACOCO_CLI_PATH, jacocoExecPath, filteredClassFilesPath, sourceFilesPath, testName, testName);
        System.out.println("Command line is: " + commandLine);
