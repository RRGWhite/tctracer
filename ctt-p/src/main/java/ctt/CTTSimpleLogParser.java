...
public class CTTSimpleLogParser {
        ...
        while ((line = br.readLine()) != null) {
            if (activeHS != null) {
                ...
                if (line.equals(">>> TEST END <<<")) {
                    ...
                    // Method invocation
                    int callDepth = getCallDepth(line) - callDepthOnTestEntry - 1; // Depth 0 == top level functions called by test
                    assert callDepth >= 0;
                ...
                if (line.startsWith(">>> TEST START <<<")) {
                    ...
                    activeHS.func = funcname.substring(funcname.lastIndexOf('.') + 1);
    ...
    private static int getCallDepth(String line) {
        ...
        for (int i = 0; i < line.length(); i++) {
            if (line.charAt(i) == ' ') callDepth++;
            else break; // bail once we see a non-space character
        }
        return callDepth;
    }
}
